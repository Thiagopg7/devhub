...
export default function Index({ stack, filter }) {
    const can = useCan();
    const { data, setData, get } = useForm({ q: filter?.q || "", status: filter?.status || "" });
    const [items, setItems] = useState(stack.data);
    ...
    const deleteConfirm = (id) => {
        confirm({
            ...
            onConfirm: () => router.delete(route("admin.stack.destroy", id), {
                preserveScroll: true,
            ...
            <AuthenticatedLayout
                header={
                    <div className="flex w-full justify-between items-center">
                        <h2 className="font-semibold text-xl leading-tight">Stack</h2>
                        {can('stack.create') && (
                            <NavButton href={route("admin.stack.create")}>Cadastrar</NavButton>
                        )}
                    </div>
                }
            >
                                        ...
                                        <SortableContext items={items.map((i) => i.id)} strategy={verticalListSortingStrategy}>
                                            <table className="w-full min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                                                ...
                                                <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
                                                    {items.map((item) => (
                                                        <SortableTr key={item.id} id={item.id} disabled={isFiltering}>
                                                            <td className="px-6 py-4">
                                                                <span
                                                                    className="inline-block w-6 h-6 text-gray-700 dark:text-gray-200"
                                                                    dangerouslySetInnerHTML={{ __html: DOMPurify.sanitize(item.icon) }}
                                                                />
                                                            </td>
                                                            <td className="px-6 py-4 font-medium dark:text-gray-100">{item.name}</td>
                                                            <td className="px-6 py-4 text-center">
                                                                <ToggleActive id={item.id} model="stack" value={item.is_active} />
                                                            </td>
                                                            <td className="px-6 py-4 text-center">
                                                                <div className="flex gap-2 justify-center">
                                                                    {!can('stack.edit') && can('stack.view') && (
                                                                        <NavButton href={route("admin.stack.edit", item.id)} title="Visualizar">
                                                                            <FaEye />
                                                                        </NavButton>
                                                                    )}
                                                                    {can('stack.edit') && (
                                                                        <NavButton href={route("admin.stack.edit", item.id)} title="Editar">
                                                                            <FaPen />
                                                                        </NavButton>
                                                                    )}
                                                                    {can('stack.delete') && (
