...
export default function Newsletter() {
    ...
    const inputCls = "w-full rounded-xl px-4 py-2.5 text-sm placeholder-[var(--text-muted)] focus:outline-none transition-colors disabled:opacity-50";
    const inputStyle = { background: 'var(--base)', border: '1px solid var(--border-2)', color: 'var(--text-bright)' };

    return (
        <section id="newsletter" className="relative" style={{ background: 'var(--base)', borderTop: '1px solid var(--border)', borderBottom: '1px solid var(--border)' }}>
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20">
                <div className="relative rounded-[22px] overflow-hidden p-8 md:p-12"
                    style={{ background: 'linear-gradient(135deg,rgba(21,36,58,0.9),rgba(16,31,48,0.9))', border: '1px solid var(--border-2)', boxShadow: '0 40px 80px -30px rgba(0,0,0,0.5)' }}>
                    ...
                    <div className="relative grid md:grid-cols-2 gap-10 items-start">
                        ...
                        <div>
                            ...
                            <p className="mt-4 text-[16px]" style={{ color: 'var(--text-body)' }}>
                            ...
                            <ul className="mt-6 space-y-3">
                                {['Curadoria semanal de conteúdo técnico', 'Tutoriais e guias antes de todo mundo', 'Cancele quando quiser, em um clique'].map((perk) => (
                                    <li key={perk} className="flex items-center gap-3 text-sm" style={{ color: 'var(--text-body)' }}>
                        ...
                        <div>
                            {submitted ? (
                                <div className="flex flex-col items-center gap-3 py-8 text-center">
                                    ...
                                    <p className="text-sm" style={{ color: 'var(--text-muted)' }}>Em breve você receberá nossas novidades.</p>
                                </div>
                            ) : (
                                <form onSubmit={handleSubmit} className="space-y-4">
                                    ...
                                    <div>
                                        <label className="block text-xs font-medium mb-1.5" style={{ color: 'var(--text-muted)' }}>Área de atuação</label>
                                    ...
                                    <label className="flex items-start gap-3 cursor-pointer">
                                        <input type="checkbox" name="lgpd_consent" checked={form.lgpd_consent} onChange={handleChange}
                                            disabled={loading}
                                            className="mt-0.5 h-4 w-4 shrink-0 rounded border-slate-500 bg-[var(--base)] text-[var(--accent)] focus:ring-[var(--accent)] focus:ring-offset-0 disabled:opacity-50" />
